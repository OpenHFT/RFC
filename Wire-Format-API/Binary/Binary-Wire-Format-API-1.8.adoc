...
[NOTE]
For\<string\> the string encode by default is a stop bit encoded len folloed by a ISO-8851-9 string, see more on this at https://github.com/OpenHFT/RFC/blob/master/Stop-Bit-Encoding/
...
|===
| type   | 7bit   | 6bit   | 5bit   | 4bit   | 3bit   | 2bit   | 1bit   | 0bit
| FieldAnchor ( 0x87 ) see examples on DeltaWire below                      | 1   | 0   | 0   | 0   | 0   | 0   | 1    |
|===
...
The following covers field caching, the first time a field is sent it is sent with a token, subsequent send of this feild will only send the token
...
|===
| (0x87) -  FIELD_ANCHOR   - special control message to denote delta wire field
| (0x00) -  ?
| (0x05) -  field len
| (0x68) - ‘h’ as Ascii
| (0x65) - ‘e’ as Ascii
...
| (0x00) -  field number tag - see section below field tag
| (0x05) -  field length
| (0x68) - ‘h’ as Ascii
| (0x65) - ‘e’ as Ascii
| (0x6C) - ‘l’ as Ascii
| (0x6C) - ‘l’ as Ascii
| (0x6F) - ‘o’ as Ascii
| (0xBA) - Field Number  - special message to denote the next value is a field number
| (0x00) - field number tag
| (0x01) - value of 1
| (0xBA) - Field Number  - special message to denote the next value is a field number
| (0x00) - field number tag
| (0x01) - value of 1
...
A Messages sent with a 'Field Anchor' should be cache locally the relationship between the field tag and its field

sp the following binary could be represent as follows :
...
[options="header"]
|===
| field number tag  | field
| 0   | hello
|===

==== Field Anchor

field can be reused, so if you are reading delta wire and you receive

[source]
87 00 05 68 65 6C 6C 6F  01 87 01 05 77 6F 72 6C 64 01                                            d·

then locally you should cache
...
[NOTE]
the value of '1' has been ignored in this example but is in the binary for completeness

===== Overwriting of the Field Anchor

lets assume later you may receive the following

[source]
87 00 08 75 6E 69 76 65  72 73 65 01

then you should overwrite the existing entry in our field cache
