...
if we write a simple non typed object, only the keys are cached.
...
| 0xBA | field number  - special message to denote the next value is a field token
| 0x87 | field anchor -  control message to denote delta wire field
| 0x01 | field token - see section below on 'Field Token Cache'
...
| |
| 0x87 | field anchor -  control message to denote delta wire field
| 0x02 | field token - see section below on 'Field Token Cache'
...
in the example above we only tokenize the field names, but if we use typed object we can also tokenize the values by the use of the (0x88) anchor tag

|===
| hex | name | descirption
| 0x88 | anchor  | used to set the Anchor in the anchor cache
| 0x89 | Updated Alias  | read/use the anchor from the anchor cache
|===

This anchor cache works exactly like the "field token cache" descibed above, however this cache is limited to an instance of the cache per object type per tcp/ip connection.

if send the message and give it a type "!MyType" then we are going to send an (0x88) anchor tag as well as a ( 0x87) field anchor like in the non typed example above
...
|hex | description
| 0x87 | Field Anchor -  control message to denote delta wire field
| 0x00 | field token - see section below on 'Field Token Cache' to 0='hello'
| 0x05 | field length
| 0x68 | ‘h’ as Ascii
| 0x65 | ‘e’ as Ascii
| 0x6C | ‘l’ as Ascii
...
|hex | description
| 0xBA | field number  - special message to denote the next value is a field token
| 0x00 | field token of 0 currently = 'hello'
| 0x89 | Updated Alias
| 0x00 | Updated Alias token - this was set via the 0x88 anchor tag, currently 0 = {  f1: hello, f2: world }

==== changing just on field

and if we were to resend the message again, but this time changed, the field f2 from 'world' to 'universe'
...
| 0x0B | len=11bytes
| 0xBA | field number  - special message to denote the next value is a field token
| 0x02 | field token of 2 currently = 'f2'
