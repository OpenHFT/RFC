...
If the same field is sent repeatably DeltaWire becomes is a more compact format than Binary Wire, so the following :
[source]
{
	hello: 1,
	hello: 1,
...
A messages sent with a 'Field Anchor', we should always cache the relationship between the field token and its field.
