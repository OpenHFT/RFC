...
A Messages sent with a 'Field Anchor', locally we  should always be cache relationship between the field token and its field.
...
so if the following binnary was received :
...
field tokens can be reused, so if we read delta wire
